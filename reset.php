...
if ($conn) { 
  ...
  $sql = "UPDATE task set attendance=0 where 1";
  mysqli_query($conn, $sql);

 // $sql="SELECT count(*) FROM information_schema.columns WHERE table_name = 'ceg'";
  ...
  while($rowmain = mysqli_fetch_array($resultmain)){
    ...
    if( $tempmain != 'task')
    {
     $sql = "SHOW COLUMNS FROM `$tempmain` ;";
     $result = mysqli_query($conn,$sql);
     while($row = mysqli_fetch_array($result)){
       $temp = $row['Field'];
       if( $temp != 'stu_names')
       {
         $sql1="ALTER TABLE `$tempmain` DROP `$temp` ;";
          ...
          <div class="col-xl-12">
            ...
            <div class="row">
              <div class="col-md-4 text-center" >
                <button class="btn btn-secondary mt-4" ><a href="index.html">Go Back</a></button>
              </div>
              <div class="col-md-4 text-center">
